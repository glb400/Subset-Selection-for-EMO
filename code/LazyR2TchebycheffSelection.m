function [Subset,res,time] = LazyR2TchebycheffSelection(PopObj,selNum,r,W)
    tic;
    a = size(PopObj, 1);
    M = size(PopObj, 2);
    selectedPop = [];
    ...
    for n = 1 : a
       ...
       [heap.hvc(n), ~] = R2Tchebycheff(currentPop,W,r);
    ...
    [~, gteArray] = R2Tchebycheff(selectedPop,W,r);    
    ...
    while size(selectedPop, 1) < selNum
        while true
            ...
            [~,gteArrayTop]=R2Tchebycheff(PopObj(top, :),W,r);
            heap.hvc(1) = mean(gteArray) - mean(min([gteArray,gteArrayTop],[],2));

            heap = heap_down_sink(1, heap);
            if(heap.index(1) == last_index)            
                selectedPop = [selectedPop; PopObj(heap.index(1), :)];

                [~,gteArrayTop]=R2Tchebycheff(PopObj(heap.index(1), :),W,r);
                gteArray=min([gteArray,gteArrayTop],[],2);
