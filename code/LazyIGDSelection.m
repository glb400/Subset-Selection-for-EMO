function [Subset,res,time] = LazyIGDSelection(PopObj,selNum)
    tic;
    a = size(PopObj, 1);
    M = size(PopObj, 2);
    selectedPop = [];
    ...
    for n = 1 : a
       ...
       [~, heap.hvc(n)] = IGDC(currentPop, PopObj, size(currentPop, 1), IGDArray);
    ...
    [IGDArray, ~] = IGDC(selectedPop, PopObj, size(selectedPop, 1), IGDArray);
    ...
    while size(selectedPop, 1) < selNum
        while true
            ...
            [~, heap.hvc(1)] = IGDC(cuurentPop, PopObj, size(cuurentPop, 1), IGDArray);
