    ...
    for t=1:T
        ...
        elseif offspringFit(2)==0
            ...
            elseif offspringChosenNum==0
                IGDArray=min(igddis(PopObj,offspringSet),[],2);
            else
                %add calculation
                ...
                for i=1:length(addSet)
                    ...
                    [IGDArray,~]=IGDCp(offspringChosenSet, PopObj, size(offspringChosenSet,1), IGDArray);
                end
                ...
                for i=1:length(deleteSet)
                    ...
                    Distance=[];
                    for j=1:n
                        Distance=[Distance;igddis(PopObj(j,:),si)];
                    end
                    ...
                    for j=1:length(recalculateSet)
                        ...
                        for k=1:offspringNum
                            IGDArray(id)=min(IGDArray(id),igddis(PopObj(id,:),PopObj(offspringElem(k),:)));
                        end
                    end
                end
            end
